  ...
  Filename:          #{check.filename}
          ...
          Dir.chdir(recipe.builddir) do
            ...
            if recipe.extracted_source
              extracted_source = recipe.builddir(recipe.extracted_source)
              Log.debug "Using custom extracted source dir: #{extracted_source}"
            end

            # Make the path to the extracted source available in the recipe.
            recipe.sourcedir = FPM::Cookery::Path.new(extracted_source)
            recipe.run_lifecycle_hook(:after_source_extraction, recipe.sourcedir)

            Log.info "Using source directory: #{recipe.sourcedir}"

            Dir.chdir(recipe.sourcedir) do
              ...
              if File.exists?(build_cookie)
                ...
                Log.info "Building in #{recipe.sourcedir}"
                recipe.build
